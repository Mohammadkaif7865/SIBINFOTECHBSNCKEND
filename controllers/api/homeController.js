...
var dashboardData = async (req, res) => {
  ...
  let category = await conn.awaitQuery(`SELECT COUNT(id) as count FROM category`);
  ...
  let contact_enquiry = await conn.awaitQuery(`SELECT COUNT(id) as count FROM contact_enquiry`);
  ...
  async.waterfall([
      ...
      function(career_id, callback) {
          if(career_id) {
            ...
            if(values.length) {
              let sql = "INSERT INTO career_experience (career_id, acomp_name_exp, acomp_level_exp, acomp_ind_exp, acomp_jtitle_exp, acomp_fyear_exp, acomp_fmonth_exp, acomp_tyear_exp, acomp_tmonth_exp, acomp_jdesc_exp, createdAt, updatedAt) VALUES ?";
      ...
      if (result) {
        res.json({error: false, message: 'Successfully sent', filePath: resumePath});
      } else {
        res.json({error: true, message: 'Something went wrong'});
      }
  });
